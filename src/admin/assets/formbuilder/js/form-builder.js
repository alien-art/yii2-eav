...
(function ($) {
  var FormBuilder = function FormBuilder(options, element) {
    ...
    var textAttribute = function textAttribute(attribute, values) {
      ...
      // Field has placeholder attribute
      if (attribute === 'Type' && !_helpers.inArray(values.type, typeFields)) {
        noMakeAttr.push(true);
      }
      ...
      if (!noMakeAttr.some(function (elem) {
        ...
        if (attribute === 'label' && _helpers.inArray(values.type, textArea)) {
          ...
          if (attribute ==='className')    
              attributefield = '<div class="form-group ' + attribute + '-wrap" style="display: none">' + attributeLabel + ' ' + attributefield + '</div>';
          else
